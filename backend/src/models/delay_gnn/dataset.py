...
class METARGraphDataset:
    ...
    def _aggregate_hour(self, rows: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        return {
            "vis_miles": vis, "wind_kts": wind, "gust_kts": gust,
            "flight_cat": flight_cat, "precip_sev": precip_sev,
            "precip_label": precip_label, "wxcodes": wxcodes, "snowdepth": snowdepth,
        }
    ...
    def _build_snapshots(self) -> List[Data]:
        ...
        prev_delays: Optional[List[float]] = None
        ...
        for hour_key in sorted_hours:
            ...
            data = Data(
                x=x, edge_index=self._edge_index.clone(),
                edge_attr=self._edge_attr.clone(), y=y, num_nodes=n,
            )
    ...
    def split(
        ...
        return self.snapshots[:train_end], self.snapshots[train_end:val_end], self.snapshots[val_end:]
